...
while($row=mysqli_fetch_array($result)){// для каждой строки из запроса
 ...
 if ($_SESSION['type'] == 2) { echo "<td><a href='delete.php?id=" . $row['id']. "'>Уничтожить</a></td>"; }
 echo "</tr>";
}
print "</table>";
...
while($row=mysqli_fetch_array($result)){// для каждой строки из запроса
 ...
 if ($_SESSION['type'] == 2) { echo "<td><a href='delete_mate.php?id=" . $row['id_mate']. "'>Уничтожить</a></td>"; } 
 echo "</tr>";
}
print "</table>";
$num_rows = mysqli_num_rows($result); // число записей в таблице БД
...
while($row=mysqli_fetch_array($result)){// для каждой строки из запроса
 ...
 if ($_SESSION['type'] == 2) { echo "<td><a href='delete_debt.php?id=" . $row['id_deb']. "'>Уничтожить</a></td>"; }
 echo "</tr>";
}
print "</table>";
$num_rows = mysqli_num_rows($result); // число записей в таблице БД
...
?>
<p> <a href="new_debt.php"> Добавить должника </a><hr>
...
<input type="submit" name="quit" value="Выйти и сменить пользователя">
</form>
<? if (isset($_POST['quit'])) {
    session_destroy();
...
if ($_SESSION['type'] == 2) { echo '<p> <a href="delete_user.php"> Уничтожить пользователя </a>'; }
